...
from utils import (
    ...
    strip_v,
    ensure_v,
    extract_brand_from_tag,
    extract_patches_version_from_tag,
    resolve_build_mode,
    gh,
    resolve,
    resolve_channels,
...
def select_apps_for_build(
    ...
    for app_table_name, app_entry in app_entries.items():
        ...
        if patches_repo not in patches_sources:
            continue
...
def run_plan():
    ...
    for app_entry in app_entries.values():
        ...
        patches_sources.setdefault(patches_repo, set()).add(patches_mode)

    active_patches_sources = set(patches_sources.keys())
    ...
    for stale_source in list(saved_versions.keys()):
        if stale_source not in active_patches_sources:
            log_sub("Cleanup")
    ...
    for patches_repo, modes in patches_sources.items():
...
def build_release_notes(built_apps, used_patches_versions, cli_entries, auth_headers):
    ...
    app_header = "Patched App 🎁" if total_apps == 1 else "Patched Apps 🎁"
    ...
    for brand in sorted(brand_apps.keys()):
        ...
        if not has_variants:
            for app_name in sorted(apps_grouped.keys(), key=app_sort_key):
                ...
                note_lines.append(
                    f"- {app_name.replace('-', ' ')}: {ensure_v(app_version_str)}"
                )
            note_lines.append("")
        else:
            for app_name in sorted(apps_grouped.keys(), key=app_sort_key):
                note_lines.append(f"### {app_name.replace('-', ' ')}")
                ...
                for variant, app_version_str in sorted(items, key=variant_sort_key):
                    if len(items) == 1 and variant is None:
                        ...
                        note_lines.append(
                            f"- {variant.replace('-', ' ')}: {ensure_v(app_version_str)}"
                        )
...
def publish_release(built_apps, release_notes, is_prerelease, brand, patches_version):
    release_tag = f"{brand}-{ensure_v(patches_version)}"
...
def cleanup_releases(config, auth_headers):
    ...
    for release in all_releases:
        ...
        brand = extract_brand_from_tag(tag)
        if not brand:
            continue
    ...
    for brand, releases in releases_by_brand.items():
        ...
        non_custom_releases = []
        for r in releases:
            ver = extract_patches_version_from_tag(r.get("tag_name") or "")
        ...
        for pre_release in pre_releases:
            pre_ver = extract_patches_version_from_tag(
                pre_release.get("tag_name") or ""
            )
            if not pre_ver:
                continue
